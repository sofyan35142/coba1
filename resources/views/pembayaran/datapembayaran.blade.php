...
<html lang="en">
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <ol class="breadcrumb mb-4">
                            ...
                            {{-- <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    <a href="/tambahpembayaran" class="btn btn-primary">Bayar</a>
                                    @endif
                                </div>
                            </div> --}}
                        <div class="card mb-4">
                            <div class="card-header">
                                ...
                                DataTabel pembayaran
                            </div>
                            <div class="form-group mt-2">
                                <a href="{{ url('/pembeli') }}" class="btn btn-primary">
                                Kembali</a>
                            ...
                            <div class="card-body">
                                <table class="table table-bordered table-dark" id="example">
                                    ...
                                    <tbody>
                                        <?php $no = 1 ?>
                                        @foreach ($datap as $datapembayaran)
                                     <tr>
                                        <td>{{  $no }}</td>
                                        <td>{{  $datapembayaran->idpembayaran->jenis_barang }}</td>
                                        <td>{{  $datapembayaran->bayar }}</td>
                                        <td>
                                            @if (auth()->user()->level=="user")
                                            <a href="/tambahrating/{{ $datapembayaran->id }}" class="btn btn-info"><i class="fa-solid fa-face-smile"></i></a>
                                            @endif
                                        @if  (auth()->user()->level=="admin")
                                        <a href="{{ url('/rating') }}" class="btn btn-info"><i class="fa-solid fa-face-smile"></i></a>
                                        @endif
                                        </td>
                                        <td>
                                        {{-- <a href="{{ url('/editsupplier/'.$datasupplier->id) }}" class="btn btn-warning"><i class="fa-solid fa-pen-to-square"></i></a> --}}
                                        <a href="#" class="btn btn-danger destroypem" data-id="{{ $datapembayaran->id }}" data-bayar="{{ $datapembayaran->bayar }}"><i class="fa-solid fa-trash"></i></a>
                                    </td>
                                    </tr>
                                    </tbody>
                                     <?php $no++ ?>
    ...
    <script>
         $('.destroypem').click(function(){
            var pembayaranid = $(this).attr('data-id');
            var bayar = $(this).attr('data-bayar');
            swal({
  title: "Yakin?",
  text: "Kamu ingin menghapus data pembayaran dengan pembayaran "+bayar+" ",
  icon: "warning",
  buttons: true,
  dangerMode: true,
})
.then((willDelete) => {
  if (willDelete) {
    window.location = "/destroypem/"+pembayaranid+""
